...
function handleSubmit(req, res) {
  ...
  if (language === "cpp") {
    ...
    dockerCommand = `docker run --rm -v ${tempDir}:/app dockerfile bash -c "javac /app/Main.java && echo '${input}' | java -cp /app Main"`;
  } else if (language === "python") {
    dockerCommand = `docker run --rm -v ${tempDir}:/app dockerfile bash -c "echo '${input}' | python3 /app/main.py"`;
  } else {
